    ...
    public class ScanSendService : IScanSendService
    {
        public ScanSendView SearchSpringData(ScanSendSearchView model)
        {
            using (var ctx = new ConXContext())
            {
                ...
                string vreq_date = mps.REQ_DATE.ToShortDateString();
                ...
                string sql = "select MPS_DET_WC.ENTITY , MPS_DET_WC.REQ_DATE , PDMODEL_MAST.SPRING_TYPE as SPRINGTYPE_CODE, MPS_DET_WC.PDSIZE_CODE , MPS_DET_WC.PDSIZE_DESC  , sum(1) as PLAN_QTY , 0 as INACT_QTY , 0 as QP_QTY   , 0 as ACT_QTY";
                sql += " FROM MPS_DET,PDMODEL_MAST,MPS_DET_WC";
                sql += " WHERE MPS_DET.req_date =  to_date(:p_req_date , 'dd/mm/yyyy')";
